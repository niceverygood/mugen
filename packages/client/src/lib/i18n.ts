...
const translations: Record<string, Record<Lang, string>> = {
  ...
  'gen.preset_label': { ko: '프리셋', ja: 'プリセット' },
  ...
  'layer.layers': { ko: '레이어', ja: 'レイヤー' },
  'layer.entities': { ko: '개 엔티티', ja: '個エンティティ' },
  'layer.show_all': { ko: '전체표시', ja: '全表示' },
  'layer.hide_all': { ko: '전체숨김', ja: '全非表示' },
  ...
  'right.step2': { ko: '② 거래처 프리셋 선택', ja: '② プリセット選択' },
  ...
  'canvas.preset_hint': { ko: '왼쪽에서 프리셋 선택 → "구조도면 자동생성" 클릭', ja: '左パネルでプリセット選択 → "構造図面自動生成" をクリック' },
  'canvas.highlight': { ko: '강조표시', ja: '強調表示' },

  // Error
  'error.no_errors': { ko: '에러 없음', ja: 'エラーなし' },
  'error.not_found': { ko: '찾을 수 없습니다', ja: '見つかりません' },
};

let currentLang: Lang = (localStorage.getItem('mugen-lang') as Lang) || 'ja';
...
// Hook-compatible: returns a t function that triggers re-render
import { useState, useCallback } from 'react';

let listeners: Array<() => void> = [];

export function useLang(): { t: (key: string) => string; lang: Lang; setLang: (lang: Lang) => void } {
  const [, forceUpdate] = useState(0);

  const subscribe = useCallback(() => {
    const handler = () => forceUpdate(n => n + 1);
    listeners.push(handler);
    return () => { listeners = listeners.filter(h => h !== handler); };
  }, []);
  ...
  return { t, lang: currentLang, setLang: changeLang };
}
